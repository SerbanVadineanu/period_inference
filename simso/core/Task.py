...
class TaskInfo(object):
    ...
    def __init__(self, name, identifier, task_type, abort_on_miss, period,
                 ...
                 et_stddev, ucet, deadline, base_cpi, followed_by,
                 list_activation_dates, preemption_cost, data, jitter, rnd_drop):
        """
        :type name: str
        :type identifier: int
        :type task_type: str
...
class GenericTask(Process):
    ...
    @property
    def ucet(self):
        return self._task_info.ucet

    @property
    def jitter(self):
        return self._task_info.jitter
...
class PTask(GenericTask):
    ...
    def execute(self):
        ...
        yield hold, self, int(self._task_info.activation_date * self._sim.cycles_per_ms
                              + random.uniform(0, self._task_info.jitter * self.period))

        while True:
            #print self.sim.now(), "activate", self.name

            # If the random drop parameter is set true we drop 15% of jobs
            if self._task_info.rnd_drop:
