...
class Configuration(object):
    ...
    def check_tasks(self):
        ...
        for index, task in enumerate(self._task_info_list):
            ...
            # UCET >= 0:
            assert task.ucet >= 0, "UCET must be positive."

            # UCET <= WCET
            assert task.ucet <= task.wcet, "UCET cannot be larger than WCET"
            ...
            # Jitter >= 0
            assert task.jitter >= 0, "Jitter must be positive"
    ...
    def add_task(self, name, identifier, task_type="Periodic",
                 ...
                 et_stddev=0, ucet=0, deadline=10, base_cpi=1.0, followed_by=None,
                 list_activation_dates=[], preemption_cost=0, data=None, jitter=0, rnd_drop=False):
        """
        ...
        task = TaskInfo(name, identifier, task_type, abort_on_miss, period,
                        ...
                        (stack_file, self.cur_dir), wcet, acet, et_stddev, ucet,
                        deadline, base_cpi, followed_by, list_activation_dates,
                        preemption_cost, data, jitter, rnd_drop)
